...
* **Операнд**: 32 бита.
* **Режимы адресации**: непосредственный, прямой абсолютный, косвенный и относительно вершины стека.
...
* **Строки (cstr)** размещаются в статической памяти данных посимвольно — один символ на 32-битное слово, в конце нулевой терминатор. В коде используется адрес начала строки.
...
* **Архитектура**: аккумуляторная (`acc`) — все вычисления идут через регистр `AC`, второй операнд берётся из памяти или из самой инструкции.
* **Ввод-вывод**: port-mapped (`port`) — обмен с портами выполняется отдельными командами `IN`/`OUT` через `AC`.
* **Поток управления**: после каждой команды `IP` инкрементируется в цикле выборки; ветвление — условными/безусловными переходами; вызов и возврат функций — через аппаратный стек; прерывания — аппаратным сохранением контекста.
...
* **Защёлки регистров** фиксируют новое значение в регистрах.
* **Мультиплексоры** выбирают источники данных.
* **Элемент `+1`** позволяет инкрементировать счётчик команд, не занимая АЛУ.
* **АЛУ** выполняет `+`, `-`, `*`, `/`, `%`, `INC`, `DEC`, `NOT` и выставляет флаги `Z` и `N`.
* **Память данных** адресуется регистром `AR`; **IO Unit** обслуживает порты, адресуемые `IO_ADDR`, по сигналам чтения/записи.

### Control Unit

Реализован как **Hardwired** на базе **FSM**. Управляющая матрица с помощью дешифратора опкода и счётчика шагов формирует на каждом такте набор управляющих сигналов.

![Control Unit](docs/control-unit.png)

Состояния процессора:
1. **FETCH**: Выборка команды из `INSTR MEM` в регистр инструкций `IR` и `IP = IP + 1`. За 1 такт.
...
1. **Запрос**: Внешнее устройство кладет данные в порт и устанавливает сигнал `IRQ` а также номер своего вектора прерывания.
...
3. **Ввод**: Происходит чтение из порта, указанного в регистре`IO_ADDR`.
...
* `compile` — транслирует исходник в бинарный файл; при указании `listing.txt` дополнительно пишет отладочный листинг.
...
* **Позднее связывание** - механизм, позволяющий транслировать тело функции отдельно от основного кода и связывать переменную с адресом функции на позднем этапе генерации кода.
  ...
  * Так как необходимо в переменную положить адрес тела функции, но он еще не определён, то адрес заменяется на `0xCCCCCCCC`, а инструкция помещается в очередь для отложенного связывания, с указанием `uid` интересующей функции.
...
| `sort` | загрузка чисел через прерывания и сортировка массива |
...
Исходник `(block (def msg "Hello") (out-str 1 msg))` транслируется в листинг (фрагмент кода и данных):
...
**CI** (GitHub Actions, `.github/workflows/ci.yml`) на каждый push и pull request запускает линтер `ruff`, проверку типов `mypy` и тесты `pytest`.